...
public class test2 {
	 ...
	 int age;
		...
		public test2(String name, int number, String team, int age) {
			super();
			this.name = name;
			this.number = number;
			this.team = team;
	 ...
	 public static void main(String[] args) {
		//1. 축구선수 인스턴스를 저장할 수 있는 List<E> 컬렉션 인스턴스를 생성해서 인스턴스를 저장하고 출력하는 프로그램을 만들어 봅시다. 
		 ...
		 List<test2> players = new ArrayList<>();
		 
		 players.add(new test2("파이아이우 마이어",13,"게르만",30));
		 players.add(new test2("슈바인 슈타이거",10,"마인츠",40));
		 players.add(new test2("마리오 고메스",9,"뮌헨 ",35));
		 players.add(new test2("니클라스 쥘레",25,"돌문 ",25));
		 players.add(new test2("니코 슐로터베크",4,"독일 ",21));
		 
		 
		for(test2 player : players) {
